...
function encodeNamespacePacket(type: string, namespace: string, payload?: unknown) {
  const hasPayload = payload !== undefined
  const body = hasPayload ? JSON.stringify(payload) : ''
  if (namespace === '/') return `${type}${body}`
  return hasPayload ? `${type}${namespace},${body}` : `${type}${namespace},`
}

function namespaceDataPrefix(type: string, namespace: string) {
...
class NativeSocketIo implements SocketLike {
  ...
  emit(event: string, ...args: any[]) {
    const packet = encodeNamespacePacket('42', this.endpoint.namespace, [event, ...args])
  ...
  disconnect() {
    ...
    this.clearTimers()
    if (this.connected) this.send(encodeNamespacePacket('41', this.endpoint.namespace))
  ...
  private handleMessage(message: string) {
    ...
    if (message.startsWith('0')) {
      this.send(encodeNamespacePacket('40', this.endpoint.namespace, this.options.auth))
      return
    }
    const connectPrefix = this.endpoint.namespace === '/' ? '40' : `40${this.endpoint.namespace}`
    ...
    const errorPrefix = namespaceDataPrefix('44', this.endpoint.namespace)
    if (message.startsWith(errorPrefix)) {
      this.emitLocal('connect_error', { message: message.slice(errorPrefix.length) })
      return
    }
    const eventPrefix = namespaceDataPrefix('42', this.endpoint.namespace)
    if (message.startsWith(eventPrefix)) {
      const json = message.slice(eventPrefix.length)
      try {
    ...
    const disconnectPrefix = this.endpoint.namespace === '/' ? '41' : `41${this.endpoint.namespace}`
    if (message.startsWith(disconnectPrefix)) {
      this.connected = false
