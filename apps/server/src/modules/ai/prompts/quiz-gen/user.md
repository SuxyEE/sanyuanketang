请根据以下要求生成**恰好 {{count}} 道**课堂测验题目（不可少于、也不可多于此数）：

- **题目数量**：{{count}} 道 · 必须严格遵守此数量 · `questions` 数组长度必须等于 {{count}}
- **知识点/主题**：{{topic}}
- **课程**：{{courseContext}}
- **难度**：{{difficulty}}
- **题目类型**：{{typesText}}
...
## 重要约束
1. **数量优先**：宁可让单题的 `analysis` / `commentPrompt` 简短一点，也必须凑齐 {{count}} 道题。
2. **输出完整 JSON**：所有 {{count}} 道题写完后再输出最后一个 `]` 和 `}` 结尾，**禁止中途截断**。
3. **类型均匀**：题型按 {{typesText}} 轮换分配，不要全是同一类型。
4. **严禁占位**：每道题必须有真实题干、真实选项、真实答案、真实解析，不允许出现"略"、"待补充"、"TODO" 等占位文字。
