...
> 版本：v1.1（2026-07-27）  
> 评估基线：`f849cd1def33188a1b8527b86df89cc1820fa6f5`  
> 初版基线：`ba2d246a4353394de0a60fbc79f4ab389a46b006`（v1.0，2026-07-23）  
...
| 教师身份 | 管理后台已接书舟统一登录，按 `(issuer, membership_id)` 绑定并自动建档 | 大屏、教师端和学生端仍走邀请码 / kiosk，未与统一身份关联 |
| 学生身份 | 本地随机身份或固定 Demo 身份 | 未与书舟 `membership_id`、智慧校园学生档案稳定绑定 |
| 课堂学情 | Gateway 内存聚合，结课按 7.1/7.2 标准信封写入 outbox | outbox 在进程内存、未落库、无重试，实际未回流智慧校园 |
| 管理端 | 登录、监控有部分真实接口；品牌与功能开关已由平台配置下发 | 班级、课程、用户、报表、AI 治理仍有大量 mock |
| 校本题库 | 已有知识点、难度、题型、审核、授权、相似题、组卷；课堂侧只有 `local-fixture` 的演示题 | 缺课堂专用服务 API、题目修订版本和课堂效果回传 |
| 智慧校园 | 已注册应用 `sanyuan-classroom` 并按学校订阅授权；课堂配置可用 `campusSchoolId` 对上校园数字 ID | 仍缺主数据同步与课堂学情投影 |
| 认证 | 管理后台走 OIDC 授权码 + PKCE 服务端换码，RS256 验签并校验 `iss`/`aud`/`exp`/`token_use` 与学校订阅；课堂自签 HS256 令牌按 `alg` 并存 | WebSocket 握手仍可关闭鉴权；合作方 partner grant 未接 |
| AI | 支持多模型和客户端传模型地址/Key | 配置权下放给客户端，存在 SSRF、密钥和额度治理风险 |

当前代码可以继续用于 Demo，但仍不是生产安全基线，至少还差三件事：`WS_AUTH_MODE` 线上仍是 `off`；
`PLATFORM_INTERNAL_TOKEN` 未配置，`/platform` 的内部接口处于空令牌放行状态；`apps/server/.env`
带着真实生产凭据进了仓库历史。
...
- **打通两边的学校标识**：智慧校园用数字 `school_id`、课堂配置用 slug，此前校园用户进来匹配不到配置会静默回退到默认学校，金林湾的老师会看到集美工业学院的品牌和题库。现在学校配置支持 `campusSchoolId` 显式对应校园数字 ID，并新增了金林湾（`campusSchoolId: "25"`）的配置；对外的 `/platform/schools/:id/config` 在明确传了学校却解析不到时返回 404，课堂上下文在解析不到时把 `schoolName` 留空，都不再拿默认学校顶替；管理端读不到配置时的兜底也改成中性文案，不再硬编码某一所学校。
...
- 金林湾的校本题库与品牌素材还是空的：`mockQuestions` 为空数组，抽题会返回空列表；logo 和主题色沿用产品默认值。
- 校园库里没有集美工业学院这所学校，`jimei-industrial` 目前只是课堂侧的演示配置，走不通统一登录。
...
## 25. 2026-07-27 方案核对记录

对着本方案逐节核对代码，共发现 6 处不一致，处理状态如下。

| # | 偏差 | 对应章节 | 状态 |
|---|---|---|---|
| 1 | 统一登录按 `sub` 绑定身份，而非 `membership_id` | 4.1 | 已修 |
| 2 | client 的 `app_code` 留空，学校订阅未校验 | 4.2 | 已修 |
| 3 | 智慧校园数字 `school_id` 与课堂 slug 对不上，且静默回退 | 4.1 / 9.2 | 已修 |
| 4 | 学情事件命名与信封不符规范 | 7.1 / 7.2 | 已修 |
| 5 | 交付仓用显示名命名，与稳定产品码口径冲突 | 22.6 | 待定 |
| 6 | 文档基线与「当前代码事实」表已过时 | 2 | 已修 |

### 25.1 学情事件对齐 7.1 / 7.2

原实现用 `lesson_ended` 这类下划线命名、无版本号，信封也缺 `schema_version` 和 `actor`，
且拿 `trace_id` 当过 `event_id` 用——两者语义完全不同，前者排查链路、后者做幂等。

- 事件名收敛到 `CLASSROOM_EVENTS` 常量，8 个事件全部按 7.2 带 `.v1` 后缀
- 对外发出的是完整信封：`event_id`（每条独立生成，供消费者幂等）、`event_type`、
  `schema_version`、`occurred_at`、`producer`、`trace_id`、`tenant_id`、`school_id`、
  `lesson_session_id`、`actor`，字段名保持 snake_case，这是跨产品契约
- 推送时额外带 `x-event-id` 头，消费方不解包也能去重
- 产品内部调用入参仍是驼峰，只有出网的信封转 snake_case

`actor` 目前是可选的：大屏、教师端和学生端还走邀请码，拿不到 `membership_id`，
所以结课事件暂时不带 actor；等这几端接入统一身份后由调用方补上。

### 25.2 第 5 项待定

内部接口和数据层的默认值确实是稳定产品码 `sanyuan-classroom`，智慧校园侧新建的应用编码也是它，
只有三个交付仓命名成了 `duoyuan-classroom-*`、包名跟着改成了显示名。要么改仓名，
要么把 22.6 的措辞放宽到「内部接口与数据层使用稳定产品码，仓库与包名可用交付名」。
